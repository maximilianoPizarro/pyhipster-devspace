<template>
  <div>
    <h2 id="page-heading" data-cy="EmployeeHeading">
      <span v-text="$t('deliveryApp.employee.home.title')" id="employee-heading">Employees</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('deliveryApp.employee.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EmployeeCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('deliveryApp.employee.home.createLabel')"> Create a new Employee </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && employees && employees.length === 0">
      <span v-text="$t('deliveryApp.employee.home.notFound')">No employees found</span>
    </div>
    <div class="table-responsive" v-if="employees && employees.length > 0">
      <table class="table table-striped table-hover" aria-describedby="employees">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('firstName')">
              <span v-text="$t('deliveryApp.employee.firstName')">First Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('lastName')">
              <span v-text="$t('deliveryApp.employee.lastName')">Last Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('deliveryApp.employee.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('phoneNumber')">
              <span v-text="$t('deliveryApp.employee.phoneNumber')">Phone Number</span>
            ...
            <th scope="row" v-on:click="changeOrder('hireDate')">
              <span v-text="$t('deliveryApp.employee.hireDate')">Hire Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('salary')">
              <span v-text="$t('deliveryApp.employee.salary')">Salary</span>
            ...
            <th scope="row" v-on:click="changeOrder('commissionPct')">
              <span v-text="$t('deliveryApp.employee.commissionPct')">Commission Pct</span>
            ...
            <th scope="row" v-on:click="changeOrder('manager.id')">
              <span v-text="$t('deliveryApp.employee.manager')">Manager</span>
            ...
            <th scope="row" v-on:click="changeOrder('department.id')">
              <span v-text="$t('deliveryApp.employee.department')">Department</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="deliveryApp.employee.delete.question" data-cy="employeeDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-employee-heading" v-text="$t('deliveryApp.employee.delete.question', { id: removeId })">
